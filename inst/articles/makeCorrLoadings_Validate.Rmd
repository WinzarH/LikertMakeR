---
title: "makeCorrLoadings() function validation"
subtitle: "How well does the function reproduce a target correlation matrix?"
date: "`r Sys.Date()`"
# author: 
#   - name: "Hume Winzar"
...
abstract: > 
 The makeCorrLoadings() function produces a correlation matrix from predefined item-factor loadings and factor correlatons. We compare the output of the function with known "true" data. Results show that the function works very well when all item-factor loadings are available. This remains the case even when factor correlations and uniqueness values are missing. 
output: 
#   pdf_document:
#     # papersize: a4paper
...
[@winzar2022]. It's purpose is to synthesise and correlate Likert-scale
...
results of *Exploratory Factor Analysis* (**EFA**) or a
*Structural Equation Model* (**SEM**). 
The resulting correlation matrix then can be applied to the `makeItems()` 
function to generate a synthetic data set of rating-scale items that closely 
resemble the original data that created the factor loadings summary table.
...
So we need an original, _True_, dataframe - preferably several different
original dataframes so that results are generalisable.

```{r}
#| eval: false

## install packages
library(LikertMakeR) # for makeCorrLoadings() function
library(rosetta) # data and some functions
library(dplyr) # data editing & manipulation
library(psych) # EFA functions
library(psychTools)
library(knitr) # table output
library(kableExtra) # table width and more
```
...
## install packages
library(LikertMakeR, warn.conflicts=F, quietly=T) # for makeCorrLoadings() function
library(rosetta, warn.conflicts=F, quietly=T) # data and some functions
library(dplyr, warn.conflicts=F, quietly=T) # data editing & manipulation
library(psych, warn.conflicts=F, quietly=T) # EFA functions
library(psychTools, warn.conflicts=F, quietly=T)
library(knitr, warn.conflicts=F, quietly=T) # table output
library(kableExtra, warn.conflicts=F, quietly=T) # table width and more
```
...
We use the _pp15 dataset_ from the `rosetta` package [@rosetta]. This is a
...
The questions and the item labels are presented in Table \ref{tab:pp_names}.
...
kable(labels, 
      ...
      caption = "\\label{tab:pp_names} Item-names - Party Panel data") |>
...
The item-correlations matrix is presented in Table \ref{tab:pp_icors}.
...
kable(pp15_cor, digits = 2, 
      # format = "latex", # booktabs = TRUE, 
      caption = "\\label{tab:pp_icors} Item correlations - Party Panel data") |>
  kable_classic(full_width = F)
```
...
Table \ref{tab:pp_floadings} presents item-factor loadings and uniquenesses for the nine items under consideration.
...
kable(factorLoadings, digits = 2, 
      # format = "latex", # booktabs = TRUE, 
      caption = "\\label{tab:pp_floadings} Item-factor loadings - Party Panel data") |>
  kable_classic(full_width = F)
```
...
Table \ref{tab:pp_fcors} presents factor correlations after Exploratory Factor Analysis.
...
kable(factorCorrs, digits = 2, 
      # format = "latex", # booktabs = TRUE, 
      caption = "\\label{tab:pp_fcors} Factor correlations - Party Panel data") |>
  kable_classic(full_width = F)
```
...
Factor loadings and factor correlations to five decimal places but without
uniquenesses.
...
'0.1', '0.2' and '0.3'. Table \ref{tab:pp_censored} presents item factor loadings with all values, and under the three different "censored" conditions.
...
kable(fl, digits = 2, align = rep("c", 8), 
      # format = "latex", # booktabs = TRUE, 
      caption = "\\label{tab:pp_censored} Censored (hidden) loadings - Party Panel data") |>
  row_spec(0, bold = TRUE) |>
  column_spec(1:9, border_left = F, border_right = F) |>
  kable_styling() |>
  add_header_above(
    header = header_text,
    align = "c",
...
With no declared uniquenesses. That is, Uniquenesses are estimated from
`1-communalities`, where: 

$$communalities = \Sigma(factor\_loadings^2)$$
...
Uniquenesses are inferred from estimated communalities, 
and we assume orthogonal factors.
...
Table \ref{tab:pp_summary} summarises results from this test, presenting Jennrich $\chi^2$ test and associated _p_-values. 
...
kable(summary_results_1, 
      # format = "latex", # booktabs = TRUE, 
      caption = "\\label{tab:pp_summary} Summary results of makeCorrLoadings() tests - Party Panel data") |>
  kable_classic(full_width = F)  
```
...
The dataset contains the following 28 variables. (The _q_ numbers are the
_SAPA_ item numbers).
...
Table \ref{tab:b5_loadings} presents item-factor loadings and uniquenesses for the Big-5 data under consideration.
...
kable(bfiLoadings, digits = 2, 
      # format = "latex", # booktabs = TRUE, 
      caption = "\\label{tab:b5_loadings} Item-factor loadings - Big 5 data") |>
  kable_classic(full_width = F)
```
...
Table \ref{tab:b5_fcors} presents factor correlations after the Exploratory Factor Analysis.
...
kable(bfiCorrs, digits = 2, 
      # format = "latex", # booktabs = TRUE, 
      caption = "\\label{tab:b5_fcors} Factor correlations - Big 5 data") |>
  kable_classic(full_width = F)
```
...
The remaining test cases are as for the first study. 
Experimental manipulations include: 
Changes to the number of decimal places considered, 
the presence of uniquenesses, 
presence of factor correlation matrix, and
...
Table \ref{tab:b5_summary} presents summary results for the tests on the Big-5 (bfi) data. 
...
kable(summary_results_2, digits = c(0, 1, 5), 
      # format = "latex", # booktabs = TRUE, 
      caption = "\\label{tab:b5_summary} Summary results - Big 5 data") |>
  kable_classic(full_width = F)
```
...
Both studies show similar and consistent results, as illustrated in Table \ref{tab:overall}.
...
kable(overall_summary, digits = c(0, 1, 3, 1, 3), 
      # format = "latex", # booktabs = TRUE, 
      caption = "\\label{tab:overall} Overall summary results") |>
...
as one might see in the results of _Exploratory Factor Analysis_ (EFA) or
_Structural Equation Modelling_ (SEM).
