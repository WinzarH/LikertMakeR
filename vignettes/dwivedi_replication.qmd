...
format: 
  ...
  pdf:
    ...
    papersize: a4
    fontsize: 11pt
    geometry: "left=2cm,right=4cm,top=2.5cm,bottom=2.5cm"
    toc: FALSE
...
# TL;DR {.unnumbered}
  ...
  * **Source study:** _Dwivedi et al. (2019)_, which reports results from a 43-item survey of 340 respondents measuring 15 constructs.
  * **Method:**
...
# Dwiveli et al. (2019) data replication {.unnumbered}
...
<!-- - reconstruct plausible datasets from **published summary statistics** -->
<!-- - generate **synthetic Likert-scale responses** with `LikertMakeR` -->
<!-- - match reported **means, standard deviations, and correlations** -->
<!-- - check whether reconstructed data reproduce the **statistical results of the original analysis** -->
<!-- ::: -->
...
::: {.column-margin}
Dwivedi A, Johnson LW, Wilkie DC, De Araujo-Gil L (2019). 
Consumer emotional brand attachment with social media 
brands and social media brand equity. 
_European Journal of Marketing_, 53(6): 1176–1204, 
doi: <https://doi.org/10.1108/EJM-09-2016-0511>
:::

In this tutorial we demonstrate the process using a published study 
by _Dwivedi et al. (2018)_. Using only the statistics reported in their 
paper, we reconstruct a synthetic dataset and show that it reproduces the 
key statistical relationships reported in the original analysis. 

_Dwevedi et al. (2019)_ sought to understand social
media platforms from a “brand” perspective through examining the effect of
consumers’ emotional attachment on social media
consumer-based brand equity (CBBE) using an online survey of 340 Australian
social media consumers, as shown in @fig-model.

<!-- ::: {.callout-tip title = "Key Idea"} -->

<!-- Even when raw data are unavailable, published studies often report enough -->
<!-- statistics to reconstruct a **plausible synthetic dataset**. -->

<!-- By matching the reported  -->
<!-- **means, standard deviations, correlations, and reliability estimates**,  -->
<!-- we can generate Likert-scale responses that reproduce -->
<!-- the statistical behaviour of the original data. -->

<!-- The **_LikertMakeR_** package automates this process. -->
...
_Dwivedi et al._ reported the correlation presented in 
...
of the items that make each variable in @tbl-moments. 
This allows us to estimate them too - thus giving us a complete set of 
...
con_items <- makeItemsScale(correlated_data$CON, 1, 5, 2, 0.87, FALSE)
names(con_items) <- c("con1", "con2")
# head(con_items)

pas_items <- makeItemsScale(correlated_data$PAS, 1, 5, 3, 0.84, FALSE)
names(pas_items) <- c("pas1", "pas2", "pas3")
# head(pas_items)

cop_items <- makeItemsScale(correlated_data$COP, 1, 5, 3, 0.80, FALSE)
names(cop_items) <- c("cop1", "cop2", "cop3")
# head(cop_items)

trt_items <- makeItemsScale(correlated_data$TRT, 1, 5, 4, 0.88, FALSE)
names(trt_items) <- c("trt1", "trt2", "trt3", "trt4")
# head(trt_items)

sat_items <- makeItemsScale(correlated_data$SAT, 1, 5, 3, 0.86, FALSE)
names(sat_items) <- c("sat1", "sat2", "sat3")
# head(sat_items)

awr_items <- makeItemsScale(correlated_data$AWR, 1, 5, 5, 0.87, FALSE)
names(awr_items) <- c("awr1", "awr2", "awr3", "awr4", "awr5")
# head(awr_items)

qul_items <- makeItemsScale(correlated_data$QUL, 1, 5, 4, 0.84, FALSE)
names(qul_items) <- c("qul1", "qul2", "qul3", "qul4")
# head(qul_items)

loy_items <- makeItemsScale(correlated_data$LOY, 1, 5, 2, 0.79, FALSE)
names(loy_items) <- c("loy1", "loy2")
# head(loy_items)

dif_items <- makeItemsScale(correlated_data$DIF, 1, 5, 2, 0.85, FALSE)
names(dif_items) <- c("dif1", "dif2")
# head(dif_items)

fit_items <- makeItemsScale(correlated_data$FIT, 1, 5, 2, 0.82, FALSE)
names(fit_items) <- c("fit1", "fit2")
# head(fit_items)

ext_items <- makeItemsScale(correlated_data$EXT, 1, 5, 2, 0.79, FALSE)
names(ext_items) <- c("ext1", "ext2")
# head(ext_items)

att_items <- makeItemsScale(correlated_data$ATT, 1, 5, 2, 0.88, FALSE)
names(att_items) <- c("att1", "att2")
# head(att_items)

rel_items <- makeItemsScale(correlated_data$REL, 1, 5, 3, 0.79, FALSE)
names(rel_items) <- c("rel1", "rel2", "rel3")
# head(rel_items)

inv_items <- makeItemsScale(correlated_data$INV, 1, 5, 4, 0.79, FALSE)
...
They are all integer responses to standard 1-5 Likert-scale-type questions.
...
The resulting dataset contains simulated item responses that follow the same statistical structure as the original study. Most achieved Cronbach's alpha are as desired as we can see in @tbl-compare_alphas. 


```{r}
#| label: tbl-compare_alphas
#| tbl-cap: Comparison between Cronbach's alphas for original published constructs and synthetic costructs.
#| tbl-cap-location: "margin"
#| echo: false
...
compared_alphas <- data.frame(
  construct = varnames,
  original = original_alphas,
  synthetic = alphas |> round(3)
)
...
in @tbl-sem_summary. We can see that not all the figures are the exactly 
...
Download this tutorial as a PDF: [Download PDF](dwivedi_replication.pdf)
