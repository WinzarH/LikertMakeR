...
##### a ten-item, seven-point Likert scale
...
```{r fig1, fig.height=3, fig.width=4, fig.align='center'}

## distribution of x

hist(x,
  cex.axis = 0.5, cex.main = 0.75,
  breaks = seq(from = 1, to = 7, by = 0.20),
...
```{r fig2, fig.height=3, fig.width=4, fig.align='center'}
...
**_lexact()_** attempts to produce a vector with exact first and second moments. It uses the *Differential Evolution* algorithm in the [**_DEoptim_**](https://cran.r-project.org/web/packages/DEoptim/index.html) package to find appropriate values within the desired constraints. 
...
**_lexact()_** can take time to complete the optimisation task. For example, **_lexact()_** executed the above example with the following sample-size & time combinations on the author's laptop [ _Windows 11 (Intel(R) i7-12700H 2.30 GHz)_ ].
...
hist(x,
  cex.axis = 0.5, cex.main = 0.75,
  breaks = seq(from = -3, to = 3, by = 0.25),
  col = "sky blue", xlab = NULL, ylab = NULL,
  main = paste("mu=", round(mean(x), 2), ", sd=", round(sd(x), 2))
)
```
...
x2 <- lexact(n, 2.5, 0.75, 1, 5, 5)
x3 <- lexact(n, 3.0, 1.50, 1, 5, 5)
...
```{r  fig7, fig.height = 7, fig.width = 7, fig.align = 'center'}
...
panel.cor <- function(x, y, digits = 2, prefix = "", cex.cor, ...) {
  ...
  text(0.5, 0.5, txt, cex = abs(3 * r))
}

panel.hist <- function(x, ...) {
  usr <- par("usr")
  par(usr = c(usr[1:2], 0, 1.5))
  h <- hist(x, breaks = seq(from = 1, to = 5, by = 0.20), plot = FALSE)
  breaks <- h$breaks
  nB <- length(breaks)
  y <- h$counts
  y <- y / max(y)
  rect(breaks[-nB], 0, breaks[-1], y, col = "sky blue", ...)
}
pairs(new3,
  upper.panel = panel.cor,
  diag.panel = panel.hist,
  cex = 1.5, pch = 21, bg = "sky blue", horOdd = TRUE,
...
The **_lcor()_** function in the example above had the following sample-size/time combinations on the author's laptop:
...
Let's generate some data: five 7-point Likert-scale items to help students decide what items should be included in a scale.

#### generate uncorrelated synthetic data with _lfast()_

```{r }

n <- 128

x1 <- lfast(n, 4.50, 1.00, 1, 7, 1)
x2 <- lfast(n, 5.25, 0.70, 1, 7, 1)
x3 <- lfast(n, 5.25, 1.25, 1, 7, 1)
x4 <- lfast(n, 2.50, 1.50, 1, 7, 1)
x5 <- lfast(n, 4.50, 1.50, 1, 7, 1)
...
tgt5 <- matrix(
  c(
    1.00, 0.75, 0.60, 0.25, -0.60,
    0.75, 1.00, 0.50, 0.25, -0.70,
    0.60, 0.50, 1.00, 0.25, -0.50,
    0.25, 0.25, 0.25, 1.00, -0.50,
    -0.60, -0.70, -0.50, -0.50, 1.00
  ),
  nrow = 5
)
```
...
```{r  fig9, fig.height = 7, fig.width = 7, fig.align = 'center'}

panel.hist <- function(x, ...) {
  usr <- par("usr")
  par(usr = c(usr[1:2], 0, 1.5))
  h <- hist(x, breaks = seq(from = 1, to = 7, by = 1), plot = FALSE)
  breaks <- h$breaks
  nB <- length(breaks)
  y <- h$counts
  y <- y / max(y)
...
pairs(new5,
  upper.panel = panel.cor,
  diag.panel = panel.hist,
  cex = 2, pch = 21, bg = rgb(0, 0, 1, 0.1), horOdd = TRUE,
  cex.labels = 2,
  font.labels = 2
)
```
