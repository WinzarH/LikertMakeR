...
<!--    fig.alt="LikertMakeR Logo"} -->
<!-- knitr::include_graphics("LikertMakeR_3.png") -->
<!-- ``` -->

<div style="text-align: center;">
  
![...](LikertMakeR_3.png){width=15%}
...
Technically, because they are bounded and not continuous, parametric statistics, such as mean, standard deviation, and correlation, should not be applied to summated rating scales. In practice, however, parametric statistics are commonly used in the social sciences because:
  ...
  2. results and conclusions drawn from technically-correct non-parametric statistics are _(almost)_ always exactly the same as for parametric statistics for such data. <br /> 
...
implying that parametric statistics are acceptable. 
...
<!-- ```{r skew, fig.align='center', echo=FALSE, out.width = '85%',  -->
<!--    fig.cap="Off-centre means always give skewed distribution in bounded rating scales", -->
<!--    fig.alt="Bounded rating-scale data with non-central means always produce skewed distributions"} -->
<!-- knitr::include_graphics("skew_chart.png") -->
<!-- ``` -->

<div style="text-align: center;">
  
![Bounded rating-scale data with non-central means always produce skewed distributions](skew_chart.png){width=75%}
...
#| fit-align: center
#| crop: TRUE
#| fig-cap: "Example: 4-item, 1-5 Likert scale"
#| fig-alt: "Histogram of 4-item, 1-5 Likert scale"
...
hist(x1,
  ...
  breaks = seq(from = (lowerbound - (1 / nItems) / 2), 
               to = (upperbound + (1 / nItems) / 2), 
               by = (1 / nItems)),
...
#| fit-align: center
#| crop: TRUE
#| fig-cap: "Example: likelihood-of-purchase scale"
#| fig-alt: "Histogram of likelihood-of-purchase scale"
...
**_lcor()_** systematically selects pairs of values in a column of 
the dataframe and swaps their places, and checks to see if this swap 
improves the correlation matrix. 
...
The resulting data look like this. Note that off-centre means cause skewed data, and large variances cause bimodal distributions.
...
#| fit-align: center
#| crop: TRUE
#| fig-cap: "Mean and variance affect the shape of scale distributions"
#| fig-alt: "Density plots show that off-centre means cause skewed data, and large variances cause bimodal distributions"

# Set break interval and breaks
break_interval <- 1 / items
breaks_seq <- seq(lowerbound, upperbound, by = break_interval)

# Calculate limits
xmin <- lowerbound - break_interval / 2
xmax <- upperbound + break_interval / 2

# Estimate densities
d1 <- density(mydat3$x1, from = xmin, to = xmax)
d2 <- density(mydat3$x2, from = xmin, to = xmax)
d3 <- density(mydat3$x3, from = xmin, to = xmax)
...
plot(xrange, yrange,
  type = "n",
  xlab = "Scale Value", ylab = "Density",
  main = "Sample data density curves"
)

# Add density curves
lines(d1, col = "#b4af46", lwd = 2)
lines(d2, col = "#4682b4", lwd = 2)
lines(d3, col = "#b4464b", lwd = 2)

# Add legend
legend("topright",
  legend = c("x1", "x2", "x3"),
  col = c("#b4af46", "#4682b4", "#b4464b"), lwd = 2, bg = "#ffffff40",
  bty = "n"
)
```
...
#| fit-align: center
#| crop: TRUE
#| fig-cap: "Histograms highlight the distribution range of variables"
#| fig-alt: "Histograms illustrate effect of mean and variance"
...
mean_x1 <- moments[1,1]
sd_x1 <- moments[2,1]
title1 <- bquote(mu == .(round(mean_x1, 2)) * "," ~ sigma == .(round(sd_x1, 2)))
plot(xrange, yrange,
     type = "n",
...
hist(mydat3$x1,
     breaks = breaks_seq, freq = FALSE, add = TRUE,
     col = "#b4af4620", border = "#b4af46"
)
lines(d1, col = "#b4af46", lwd = 2)
## end plot x1
##
...
mean_x2 <- moments[1,2]
sd_x2 <- moments[2,2]
title2 <- bquote(mu == .(round(mean_x2, 2)) * "," ~ sigma == .(round(sd_x2, 2)))
plot(xrange, yrange,
     type = "n",
...
mean_x3 <- moments[1,3]
sd_x3 <- moments[2,3]
title3 <- bquote(mu == .(round(mean_x3, 2)) * "," ~ sigma == .(round(sd_x3, 2)))
plot(xrange, yrange,
     type = "n",
     xlab = "x3", ylab = "Density",
     main = title3
)
hist(mydat3$x3,
     breaks = breaks_seq, freq = FALSE, add = TRUE,
     col = "#b4464b20", border = "#b4464b"
)
lines(d3, col = "#b4464b", lwd = 2)
## end plot x3
...
#| fig-align: center
#| crop: TRUE
#| fig-cap: "Scree plot from makeCorrAlpha() example"
#| fig-alt: "Scree plot of eigenvalues"
...
var_names <- sprintf("v%02d", 1:12)
colnames(cor_matrix_12) <- var_names
rownames(cor_matrix_12) <- var_names
```
...
Eigenvalues and Scree plot of the Correlation Matrix
...
#| crop: TRUE
#| fig-cap: "Scree plot from makeCorrAlpha() example"
#| fig-alt: "Cronbach's Alpha and Scree plot of eigenvalues"
#| 
...
colnames(factorLoadings) <- factor_names
...
rownames(factorCor) <- factor_names
colnames(factorCor) <- factor_names
```
  
The factor loadings matrix looks like this: 
  
```{r print_factor_loadings}
#| echo: FALSE
...
And the factor correlation matrix looks like this:
...
#| fig-align: center
#| crop: true
#| fig-cap: "Scree plot of item-correlation matrix"
#| fig-alt: "Scree plot of item-correlation matrix"
...
#| fig-align: center
#| crop: true
#| fig-cap: "Scree plot of orthogonal-items correlation matrix"
#| fig-alt: "Scree plot of orthogonal-items correlation matrix"
#| 
...
#| fig-align: center
#| crop: true
#| fig-cap: "Summary of dataframe from makeItems() function"
#| fig-alt: "Correlation matrix"
...
# Upper panel with regression line
upper.panel <- function(x, y) {
  points(x, y, pch = 19, col = "#4682b410")
  # Fit a linear model
  fit <- lm(y ~ x)
  abline(fit, col = "#ff8000", lwd = 2)
}
...
panel.hist <- function(x, ...) {
  ...
  # Define custom breaks: make bins around each integer
  breaks <- seq(floor(min(x)) - 0.5, ceiling(max(x)) + 0.5, by = 1)
  #   breaks <- h$breaks
...
```{r summatedScale_histogram}
#| echo: FALSE
#| fig-height: 5
#| fig-width: 5
#| fig-align: center
#| crop: TRUE
#| fig-cap: "Summated scale distribution"
#| fig-alt: "Histogram of summated scale"
...
#| fig-align: center
#| crop: TRUE
#| fig-cap: "Scree plot of new items 1"
#| fig-alt: "Scree plot of eigenvalues"
...
#| fig-align: center
#| crop: TRUE
#| fig-cap: "Scree plot of new items 2"
#| fig-alt: "Scree plot of eigenvalues"
...
#| fig-align: center
#| crop: TRUE
#| fig-cap: "Scree plot of new items 3"
#| fig-alt: "Scree plot of eigenvalues"
...
#| fig-align: center
#| crop: TRUE
#| fig-cap: "Visualising data after t-test"
#| fig-alt: "boxplot of two data sets"
...
boxplot(values ~ group,
  data = df,
  main = paste0(
    "Boxplot: x1 & x2\nt = ", t_result$statistic |> round(2),
    ", p = ", t_result$p.value |> round(2)
  ),
  xlab = paste0("n(x1) = ", length(x1), ", n(x2) = ", length(x2)),
  ylab = "Scale Values",
  col = c("lightblue", "skyblue")
)
```
...
Paired t-tests apply to observations that we expect to be associated with each other. 
For example: the same people rating an object before and after a treatment; 
...
```{r makePaired_output_properties}
#| echo: TRUE
...
The data can be visualised like the following chart. 
See, in this example, how most responses rise over time, 
with some falling or staying the same, 
so that the average rise is about 0.5.
...
#| fig-align: center
#| crop: TRUE
#| fig-cap: "Visualising paired-sample t-test data"
#| fig-alt: "boxplot of paired-sample t-test data"

# Set up plot
plot(c(1, 2), range(pairedDat),
  type = "n",
  xlab = "", ylab = "Score",
  xaxt = "n",
  frame.plot = FALSE,
  main = paste0(
    "Change Over Time with Boxplots",
    "\npaired-t = ", paired_t$statistic |> round(2),
    ", p = ", paired_t$p.value |> round(2),
    ", n = ", nrow(pairedDat)
  )
)

axis(1, at = c(1.05, 1.95), labels = c("Time 1", "Time 2"))

# Overlay boxplots
boxplot(list(Time1 = pairedDat$V1),
  at = 1.05,
  add = TRUE,
  boxwex = 0.2,
  # col = "#4682b430",
...
boxplot(list(Time2 = pairedDat$V2),
  at = 1.95,
  add = TRUE,
  boxwex = 0.2,
  # col = "#4682b430",
...
for (i in 1:nrow(pairedDat)) {
  lines(c(1.05, 1.95), c(pairedDat$V1[i], pairedDat$V2[i]),
    type = "b", pch = 16, col = "#a9a9a9"
  )
}
```
...
We generate three attitude scales, each with different numbers of items, 
and different _Cronbach's Alpha_ values. 
Then we generate a "behavioural intention" scale: an 11-point scale similar 
to a _Juster scale_.
 
```{r correlateScales_dataframes}
#| echo: TRUE
...
Here we calculate means, standard deviations and correlations among items within each attitude scale, to compare with the predefined parameters.
...
Define a correlation matrix for the scales: four scales means a 4x4 correlation matrix.
And gather our example scale dataframes into a list.
...
Note that the `dataframes` parameter refers to a list of named dataframes.
...
#| fig-align: center
#| crop: TRUE
#| fig-cap: "Correlations of new scale items"
#| fig-alt: "Correlation Matrix"
...
panel.hist <- function(x, ...) {
  ...
  # Define custom breaks: make bins around each integer
  breaks <- seq(floor(min(x)) - 0.5, ceiling(max(x)) + 0.5, by = 1)
  h <- hist(x, breaks = breaks, plot = FALSE) # << specify breaks manually
  nB <- length(breaks)
  y <- h$counts
  y <- y / max(y)
...
<!-- Eigenvalues of all generated data -->
...
<!-- #| fig-align: center -->
<!-- #| crop: TRUE -->
<!-- #| fig-cap: "All variables" -->
<!-- #| fig-alt: "Scree plot of eigenvalues" -->


<!-- ## eigenvalues of dataframe correlations -->
<!-- cor_correlated_scales <- cor(my_correlated_scales) -->
<!-- eigenvalues(cormatrix = cor_correlated_scales, scree = TRUE) |>  -->
<!--   round(2) -->
<!-- ``` -->

Eigenvalues of Predictor variables (Attitude items) only
...
#| fig-align: center
#| crop: TRUE
#| fig-cap: "Predictor variable items only"
#| fig-alt: "Scree plot of eigenvalues"


### Eigenvalues of predictor variable items only
predictor_cors <- cor(my_correlated_scales[, -16])
eigenvalues(cormatrix = predictor_cors, scree = TRUE) |> 
  round(2)
```
...
Winzar, H. (2025). LikertMakeR: Synthesise and Correlate Likert Scale and Rating-Scale Data Based on Summary Statistics Only. CRAN: 
