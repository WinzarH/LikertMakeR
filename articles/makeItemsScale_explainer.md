...
Here, the resulting *Cronbach’s alpha* = 0.8111, so the synthetic data
...
\\ score = alpha\\err^2 + w\\balance \* balance\\penalty \* (1 +
alpha\\err) \\

where:

- \\alpha\\err = abs(calculated\\alpha - target\\alpha)\\
- \\balance\\penalty = mean((col\\means - expected\\mean)^2)\\
- \\w\\balance\\ is a relative weighting coefficient

Lower values for \\score\\ indicate better solutions.

\\balance\\penalty\\ measures how evenly the item means are distributed.
