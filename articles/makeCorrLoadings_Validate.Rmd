...
abstract: > 
 makeCorrLoadings() produces a correlation matrix from published item-factor loadings and factor loadings. We test how well the function reproduces the true correlation matrix compared with known "true" data. Results show that the function works very well when all item-factor loadings are available. This remains the case even when factor correlations and uniqueness values are missing. 
format: 
  html: default
  # pdf:
  #   papersize: A4
...
A validate `makeCorrLoadings()` function should be able to produce a
...
So we need an original, True, dataframe, preferably several different
